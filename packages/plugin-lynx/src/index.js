import {Blockchains, Network, Plugin, PluginTypes, SocketService, WALLET_METHODS} from '../../plugin-arisenjs/src/node_modules/@arisenidjs/core';
...
export default class ArisenidLynx extends Plugin {
    ...
    constructor(eosjs){
    	...
    	if(!eosjs) {
    		console.error('Lynx Plugin: You must pass in an eosjs version. Either ({Api, JsonRpc}) for eosjs2 or (Arisen) for eosjs1');
    		return;
	    }
	    this.name = 'Lynx';
	    this.isEosjs2 = false;
        ...
        return {
	        ...
	        [WALLET_METHODS.requestSignature]:async ({abis, transaction, network}) => {
		        ...
		        if(this.isEosjs2){
		        	...
		        	const arisen = new this.eosjs({httpEndpoint:Network.fromJson(network).fullhost(), chainId:network.chainId});
		        	let abis = {};
